-- Corrige a quitação de parcelas pagas no vencimento quando existe multa configurada.
...
begin
  ...
  loop
    ...
    if v_row_total > 0 then
      ...
      v_new_status := case
        when v_new_paid >= r.amount
         and (v_days_late = 0 or (
           (coalesce(r.late_interest_paid,0)+v_late_interest_part) >= v_late_interest_accrued
           and (coalesce(r.late_fee_paid,0)+v_fee_part) >= coalesce(r.late_fee_base,0)
         )) then 'paid'
        else 'partial'
      end;
