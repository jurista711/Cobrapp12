...
class _CalculatorPageState extends State<CalculatorPage> {
  ...
  bool _syncingDerivedField = false;
  ...
  void initState() {
    super.initState();
    for (final c in [_principal, _installments, _rate, _desiredInstallment]) {
      c.addListener(_refreshDerivedField);
    }
  }
  ...
  void dispose() {
    for (final c in [_principal, _installments, _rate, _desiredInstallment]) {
      c.removeListener(_refreshDerivedField);
    }
    for (final c in [
      _principal,
      _installments,
  ...
  void _refreshDerivedField() {
    if (_syncingDerivedField) return;
    final principal = _number(_principal);
    final n = _integer(_installments);
    if (principal == null || principal <= 0 || n == null || n <= 0) return;

    _syncingDerivedField = true;
    try {
      if (_target == _TargetField.rate) {
        final rate = _number(_rate);
        if (rate == null || rate < 0 || rate > 9999) return;
        final payment = _paymentFor(principal, rate, n);
        final formatted = _decimal(payment);
        if (_desiredInstallment.text != formatted) {
          _desiredInstallment.value = TextEditingValue(
            text: formatted,
        ...
        final rate = _rateFromDesired(principal, desired, n);
        if (!rate.isFinite || rate < 0 || rate > 9999) return;
        final formatted = _decimal(rate);
        if (_rate.text != formatted) {
          _rate.value = TextEditingValue(
            text: formatted,
            selection: TextSelection.collapsed(offset: formatted.length),
          );
        }
      }
      if (_showSimulation && mounted) {
        setState(() => _showSimulation = false);
      }
    } finally {
      _syncingDerivedField = false;
    }
  }

  void _swapTarget() {
    setState(() {
      _target = _target == _TargetField.rate ? _TargetField.installment : _TargetField.rate;
      _showSimulation = false;
    });
    _refreshDerivedField();
  }

  double _paymentFor(double principal, double ratePercent, int n) {
    switch (_interestMode) {
      case _InterestMode.principal:
        return (principal + principal * ratePercent / 100) / n;
      case _InterestMode.perInstallment:
        return principal / n + principal * ratePercent / 100;
      case _InterestMode.compoundBank:
        return _compoundPayment(principal, ratePercent, n);
    }
  }

  double _rateFromDesired(double principal, double payment, int n) {
    switch (_interestMode) {
      case _InterestMode.principal:
        return ((payment * n / principal) - 1) * 100;
      case _InterestMode.perInstallment:
        return ((payment - principal / n) / principal) * 100;
      case _InterestMode.compoundBank:
        return _rateForPayment(principal, payment, n);
    }
  }
  ...
  DateTime _nextDate(DateTime current) {
    switch (_frequency) {
      ...
      case _PaymentFrequency.monthly:
        final monthStart = DateTime(current.year, current.month + 1, 1);
        return DateTime(
          monthStart.year,
          monthStart.month,
          math.min(current.day, DateUtils.getDaysInMonth(monthStart.year, monthStart.month)),
        );
      case _PaymentFrequency.manual:
      ...
      case _PaymentFrequency.monthDays:
        ...
        final target = later.isNotEmpty
            ? DateTime(current.year, current.month, 1)
            : DateTime(current.year, current.month + 1, 1);
        return DateTime(
          target.year,
          target.month,
          math.min(day, DateUtils.getDaysInMonth(target.year, target.month)),
        );
    }
  }
  ...
  void _simulate() {
    ...
    if (_target == _TargetField.installment) {
      ...
      rate = _rateFromDesired(principal, desired, n);
      if (!rate.isFinite || rate < 0 || rate > 9999) {
        setState(() => _error = 'A parcela desejada não gera uma taxa válida.');
        return;
      }
      _syncingDerivedField = true;
      _rate.text = _decimal(rate);
      _syncingDerivedField = false;
    }
    ...
    if (_lateInterest) {
      ...
      if (_graceDays.text.trim().isEmpty) {
        setState(() => _error = 'Os dias de carência são obrigatórios');
        return;
      }
      if (_lateRate.text.trim().isEmpty) {
        setState(() => _error = 'A taxa de juros de mora é obrigatória');
        return;
      }
    ...
    final principalInterest = principal * rate / 100;
    ...
    for (var i = 1; i <= n; i++) {
      ...
      if (_interestMode == _InterestMode.perInstallment) {
        ...
        rowInterest = principalInterest / n;
        rowCapital = principal / n;
      }
      balance = math.max(0, balance - rowCapital);
      rows.add(_PaymentRow(
        number: i,
        due: due,
        capital: rowCapital,
        interest: rowInterest,
        total: rowCapital + rowInterest,
      ));
    }

    final totalInterest = rows.fold<double>(0, (sum, row) => sum + row.interest);
      ...
      children: [
            ...
            child: Column(
              children: [
                ...
                TextField(
                  ...
                  decoration: _dec('Valor', icon: Icons.attach_money_rounded, prefix: 'R\$ '),
                ),
                const SizedBox(height: 10),
                TextField(
                  controller: _installments,
                  keyboardType: TextInputType.number,
                  decoration: _dec('Cotas', icon: Icons.format_list_numbered_rounded),
                ),
                const SizedBox(height: 10),
                Row(
                  children: [
                    Expanded(
                      child: TextField(
                        controller: _rate,
                        enabled: _target == _TargetField.rate,
                        keyboardType: const TextInputType.numberWithOptions(decimal: true),
                        decoration: _dec('Juros do crédito', icon: Icons.percent_rounded),
                      ),
                    ),
                    const SizedBox(width: 8),
                    IconButton.filledTonal(
                      onPressed: _swapTarget,
                      tooltip: 'Alternar campo calculado',
                      icon: const Icon(Icons.swap_horiz_rounded),
                    ),
                    const SizedBox(width: 8),
                    Expanded(
                      child: TextField(
                        controller: _desiredInstallment,
                        enabled: _target == _TargetField.installment,
                        keyboardType: const TextInputType.numberWithOptions(decimal: true),
                        decoration: _dec('Parcela desejada', icon: Icons.payments_rounded, prefix: 'R\$ '),
                      ),
                    ),
                  ],
                ),
                Align(
                  ...
                  child: Text(
                    _target == _TargetField.rate ? 'A taxa é a referência' : 'A parcela desejada é a referência',
                ...
                if (_frequency == _PaymentFrequency.manual) ...[
                  const SizedBox(height: 10),
                  TextField(
                    controller: _manualDays,
                    keyboardType: TextInputType.number,
                    onChanged: (_) => setState(() => _showSimulation = false),
                    decoration: _dec('Intervalo entre pagamentos (dias)', icon: Icons.today_rounded),
                  ),
                ],
                if (_frequency == _PaymentFrequency.monthDays) ...[
                  const SizedBox(height: 10),
                  TextField(
                    controller: _monthDays,
                    keyboardType: TextInputType.text,
                    onChanged: (_) => setState(() => _showSimulation = false),
                    decoration: _dec('Dias do mês', icon: Icons.calendar_month_rounded),
                    maxLength: 30,
                  ),
                  Align(
                    alignment: Alignment.centerLeft,
                    child: Text('Exemplo: 5, 15, 25', style: TextStyle(fontSize: 11, color: cs.onSurfaceVariant)),
                  ),
                ],
                const SizedBox(height: 10),
                InkWell(
                  ...
                  child: InputDecorator(
                    decoration: _dec('Data do Crédito', icon: Icons.calendar_today_rounded),
                    child: Text(date(_creditDate)),
                  ),
                ),
                ...
                SwitchListTile.adaptive(
                  ...
                  subtitle: Text(
                    'Cobrar juros sobre valores em atraso após a carência.',
                    style: TextStyle(fontSize: 11, color: cs.onSurfaceVariant),
                  ),
                  value: _lateInterest,
                  onChanged: (v) => setState(() {
                ...
                if (_lateInterest) ...[
                  TextField(
                    ...
                    keyboardType: TextInputType.number,
                    decoration: _dec('Dias de Carência', icon: Icons.calendar_today_rounded),
                  ),
                  const SizedBox(height: 10),
                  TextField(
                    controller: _lateRate,
                    keyboardType: const TextInputType.numberWithOptions(decimal: true),
                    decoration: _dec('Taxa de Juros de Mora (%)', icon: Icons.percent_rounded),
                  ),
                  const SizedBox(height: 10),
                  TextField(
                    controller: _lateFee,
                    keyboardType: const TextInputType.numberWithOptions(decimal: true),
                    decoration: _dec('Valor da Multa por Atraso', icon: Icons.attach_money_rounded, prefix: 'R\$ '),
                  ),
                  const SizedBox(height: 6),
                  Align(
                    alignment: Alignment.centerLeft,
                    child: Text(
                      'Exemplo: uma parcela em atraso aplica a taxa de mora após a carência e soma a multa.',
                      style: TextStyle(fontSize: 11, color: cs.onSurfaceVariant),
                    ),
                  ),
                ],
        ...
        if (_showSimulation) ...[
          ...
          _SummaryCard(
            ...
            firstDue: _plan.first.due,
            lastDue: _plan.last.due,
            principal: _total! - _interest!,
            interest: _interest!,
            payment: _payment!,
            total: _total!,
            rate: _effectiveRate!,
            installments: _plan.length,
            frequency: _frequencyLabel,
            note: _addNote ? _note.text : null,
          ),
          const SizedBox(height: 12),
          Card(
            child: Padding(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            _line('Data do empréstimo', date(creditDate)),
            _line('Próxima data de pagamento', date(firstDue)),
            _line('Data de vencimento do empréstimo', date(lastDue)),
            ...
            _line('Montante total do empréstimo', money(principal)),
            _line('Empréstimo + juros', money(total)),
            _line('Total pago', money(0)),
            const Divider(height: 18),
            _line('Dívida de Capital', money(principal)),
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Text(
          'Capital ${money(row.capital)} • Juros ${money(row.interest)}\n'
