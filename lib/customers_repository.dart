...
class CustomersRepository {
  ...
  Future<List<Map<String, dynamic>>> allCustomers() async {
    final rows = await db.from('cobrapp_customers').select().eq('user_id', uid).order('name');
    return List<Map<String, dynamic>>.from(rows);
  }

  Future<int> customerCount() async {
    final rows = await db.from('cobrapp_customers').select('id').eq('user_id', uid);
  ...
  Future<void> addCustomer(Map<String, dynamic> values) async {
    if (!await canAddCustomer()) throw StateError('Limite de clientes do plano atual atingido.');
    await db.from('cobrapp_customers').insert({'user_id': uid, ..._customerPayload(values)});
  }

  Future<void> updateCustomer(String id, Map<String, dynamic> values) async {
    await db.from('cobrapp_customers').update(_customerPayload(values)).eq('id', id).eq('user_id', uid);
  }
  ...
  Map<String, dynamic> _customerPayload(Map<String, dynamic> values) => {
    ...
    'tags': values['tags'] is List ? List<String>.from(values['tags']) : cleanTags(values['tags']?.toString()),
  ...
  Future<void> deleteCustomer(String id) async =>
      db.from('cobrapp_customers').delete().eq('id', id).eq('user_id', uid);

  Future<void> setActive(String id, bool active) async =>
      db.from('cobrapp_customers').update({'is_active': active}).eq('id', id).eq('user_id', uid);

  Future<void> saveSignature(String id, String? signatureData) async =>
      db.from('cobrapp_customers').update({'signature_data': clean(signatureData)}).eq('id', id).eq('user_id', uid);

  Future<Map<String, int>> customersPerDay() async {
    final rows = await db.from('cobrapp_customers').select('created_at').eq('user_id', uid).order('created_at', ascending: false);
  ...
  Future<List<Map<String, dynamic>>> customersMostPaid() async {
    final customers = await db.from('cobrapp_customers').select('id,name,last_name').eq('user_id', uid);
    final payments = await db.from('cobrapp_payments').select('customer_id,amount').eq('user_id', uid);
    final names = <String, String>{};
    for (final raw in customers as List) {
      final row = Map<String, dynamic>.from(raw as Map);
      final first = row['name']?.toString() ?? '';
      final last = row['last_name']?.toString() ?? '';
      names[row['id'].toString()] = [first, last].where((e) => e.trim().isNotEmpty).join(' ').trim();
    }
    ...
    final result = [for (final entry in totals.entries) {'customer_id': entry.key, 'name': names[entry.key] ?? 'Cliente', 'total_paid': entry.value}];
    result.sort((a, b) => (b['total_paid'] as double).compareTo(a['total_paid'] as double));
    return result;
  }
  ...
  Future<int> importCustomers(List<Map<String, dynamic>> items) async {
    ...
    for (final item in items) {
      if (limit != null && limit > 0 && current >= limit) break;
      final name = (item['name'] ?? '').toString().trim();
      if (name.isEmpty) continue;
      allowed.add({'user_id': uid, ..._customerPayload(item)});
      current++;
    }
    if (allowed.isEmpty && items.isNotEmpty && limit != null && limit > 0) {
      throw StateError('Limite de clientes do plano atual atingido.');
    }
    if (allowed.isNotEmpty) await db.from('cobrapp_customers').insert(allowed);
